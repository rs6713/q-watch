...
class PeoplePanel(ttk.Frame):
    def __init__(self, parent, people=None):
        tk.Frame.__init__(self, parent)
        self.notebook = ttk.Notebook(self, height=300)
        self.personPage = ttk.Frame(self.notebook)
        self.characterPage = ttk.Frame(self.notebook)
        ...
        personHeader = ttk.Frame(self.personPage)
        personHeader.pack(side="top", padx=5, pady=5, fill=tk.X)
        ttk.Label(personHeader, text="Insert People working on movie").pack(
            side="left", padx=(0, 10))
        ttk.Button(personHeader, text="+", command=self.create_person).pack(
            side="left", pady=5, padx=5
        )
        self.personContainer = ttk.Frame(self.personPage,  # highlightbackground="black", highlightthickness=2
                                         )
    ...
    def create_person(self, person=None):
        ...
        # First name
        ttk.Label(newPerson, text="First Name").grid(
            column=0, row=0, sticky=tk.W)
        firstname = ttk.Entry(newPerson, width=30)
        firstname.focus()
        firstname.grid(column=1, row=0, sticky=tk.W, padx=(0, 10), pady=(0, 5))
        if "FIRST_NAME" in person:
            firstname.insert(0, person.FIRST_NAME)

        # Last Name
        ttk.Label(newPerson, text="Last Name").grid(
            column=0, row=1, sticky=tk.W)
        lastname = ttk.Entry(newPerson, width=30)
        lastname.grid(column=1, row=1, sticky=tk.W, padx=(0, 10), pady=(0, 5))
        if "LAST_NAME" in person:
            lastname.insert(0, person.LAST_NAME)

        # DOB
        ttk.Label(newPerson, text="DOB").grid(column=0, row=2, sticky=tk.W)
        dob = ttk.Entry(newPerson, width=30)
        dob.grid(column=1, row=2, sticky=tk.W, padx=(0, 10), pady=(0, 5))
        if "DOB" in person:
        ...
        save_button = ttk.Button(
            newPerson, text="Save", width=10, command=save_person, style='Accent.TButton')
        save_button.grid(column=3, padx=(20, 0), sticky=tk.E, row=0)

        exit_button = ttk.Button(
            newPerson, text="Cancel", width=10, command=newPerson.destroy)
        exit_button.grid(column=3, padx=(20, 0), sticky=tk.E, row=1)

        button_frame = ttk.Frame(newPerson)
        ...
        role_menu.pack(side="left", fill=tk.X, expand=True, padx=(0, 5))

        sexuality_menu = MenuMultiSelector(
            button_frame, "Sexuality", self.sexualities)
        sexuality_menu.pack(side="left", fill=tk.X, expand=True, padx=(0, 5))

        gender_menu = MenuMultiSelector(button_frame, "Gender", self.genders)
        gender_menu.pack(side="left", fill=tk.X, expand=True, padx=(0, 5))

        ethnicity_menu = MenuMultiSelector(
            button_frame, "Ethnicity", self.ethnicities)

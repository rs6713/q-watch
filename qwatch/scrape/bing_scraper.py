...
def user_input():
    ...
    if object_check['config_file'] != '':
        ...
        parser.add_argument('-cd', '--chromedriver',
                            help='chromedriver path', type=str, default='./chromedriver')
        ...
        parser.add_argument('--download', default=False,
                            help='Download images', action="store_true")
        ...
        parser.add_argument('--search', type=str, default='',
                            help='search string, i.e. bees on flowers')
        ...
        if args.search:  # construct url
            args.url = 'https://www.bing.com/images/search?q=%s' % args.search.replace(
                ' ', '%20')
...
class googleimagesdownload:
    ...
    def download_extended_page(self, url, chromedriver):
        ...
        pbar = tqdm(enumerate(range(30)),
                    desc='Downloading HTML...', total=30)  # progress bar
        for _ in pbar:
            try:  # click 'see more' button if found
                # browser.find_element_by_id("smb").click()  # google images 'see more' button

                # bing images 'see more' button
                browser.find_element(By.CLASS_NAME, "btn_seemore").click()
            except:
                pass
            pbar.desc = 'Downloading HTML... %d elements' % len(
                browser.page_source)  # page source
            ...
            try:
                ...
                if format:
                    if not image_format or image_format != format:
                        ...
                        return_image_name = ''
                        absolute_path = ''
                        download_message = '%s %s' % (
                            image_url, download_message)
    ...
    def _get_next_item(self, s):
        ...
        if start_line == -1:  # If no links are found then give an error!
            ...
            try:
                object_decode = bytes(html.unescape(
                    ...
                    if arguments['related_images']:
                        ...
                        for key, value in tabs.items():
                            ...
                            self.create_directories(
                                main_directory, final_search_term)
